...
function fetchAndUpdatePriceETH() {
    fetch(`https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd&cachebust=${new Date().getTime()}`).then(data => data.json()).then(data => {
        let newPrice = Math.floor(data.ethereum.usd);
        if(lastPrice !== 0) {
        ...
        lastPrice = newPrice;
        chrome.browserAction.setBadgeText({text: newPrice.toString()});
        setTimeout(() => {
